...
@Singleton
class ApiV3Controller @Inject()(appConfig: AppConfig, accessChecker: AccessChecker, mcc: MessagesControllerComponents, journeyRepository: JourneyRepository, val authConnector: AuthConnector) extends FrontendApiController(mcc) with AuthorisedFunctions {
  ...
  private def beginJourney(authProviderId: String, init: InitRequest) = {
    ...
    journeyRepository
      ...
      .map { journeyId =>
        ...
        val completeUrl = api.routes.ApiV3Controller.complete(journeyId.toHexString).url
              ...
              .map { x =>
                x.flatMap { j =>
                  if (j.authProviderId.isEmpty || j.authProviderId.get == authProviderId) {
                    Session.toCompleteV3ResponseJson(j.data)
                  } else None
                }
              }
              .map {
