...
class JourneyRepositoryITSpec extends AnyWordSpec with Matchers with GuiceOneServerPerSuite with MockitoSugar {
  ...
  "Updating personal account details" should {
    ...
    "handle unsetting the roll number" in {
      ...
      val journey = Journey(journeyId, Some("1234"), Instant.now.plus(1, ChronoUnit.HOURS), "serviceIdentifier", "continueUrl", session, None, None, None, Option[false])
  ...
  "Can store a typical journey" should {
    ...
    "handle changing the account type" in {
      ...
      val businessSession = BusinessAccountDetails(Some("companyName"), Some("sortCode"), Some("accountNumber"), Some("rollNumber"), iban = Some("some-iban"), matchedAccountName = None)
      val session = Session(accountType = Some(Business), address = None, personal = None, business = Some(businessSession))
      val journey = Journey(journeyId, Some("1234"), Instant.now.plus(1, ChronoUnit.HOURS), "serviceIdentifier", "continueUrl", session, Some(messages), None, None, Option[false])
      await(repository.insertRaw(journey))
