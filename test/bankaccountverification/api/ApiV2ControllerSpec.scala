...
class ApiV2ControllerSpec extends AnyWordSpec with Matchers with MockitoSugar with GuiceOneAppPerSuite with OptionValues {
  ...
  private lazy val controller = app.injector.instanceOf[ApiV2Controller]
    ...
    "return 200" when {
      "A continueUrl is provided" in {
        ...
        initResponse.completeUrl shouldBe
          bankaccountverification.api.routes.ApiV2Controller.complete(initResponse.journeyId).url
